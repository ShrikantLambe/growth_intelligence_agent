...
def _load(filename: str) -> pd.DataFrame:
    ...
    if df.empty:
        logger.warning("%s is empty", filename)
    return df
...
def get_metric(query: str) -> str:
    ...
    q = query.strip().lower()[:200]  # guard against excessively long inputs
    ...
    if "all" in q:
        ...
        if matched_name:
            ...
            result = df[df["metric_name"].str.lower().str.contains(re.escape(q), na=False)].to_dict(orient="records")

    if not result:
        return json.dumps({"message": f"No metrics found matching: {query}", "available": df["metric_name"].unique().tolist()})

    logger.debug("get_metric('%s') → %d rows", query, len(result))
...
def get_pipeline_by_segment(query: str = "all") -> str:
    ...
    def summarize(df, group_col):
        ...
        for val, grp in df.groupby(group_col):
            ...
            won_pipeline = grp[grp["stage"] == "Closed Won"]["pipeline_value"]
            summary.append({
                "segment": str(val),
                ...
                "avg_deal_size": int(won_pipeline.mean()) if len(won_pipeline) > 0 else 0,
            })
        return summary
    ...
    if "region" in q:
        ...
        avg_open = open_opps["pipeline_value"].mean() if len(open_opps) > 0 else 0
        result = {
            "total_open_pipeline": int(open_opps["pipeline_value"].sum()),
            "open_deal_count": len(open_opps),
            "avg_open_deal_size": int(avg_open),
...
def get_product_usage(query: str = "summary") -> str:
    ...
    if "at-risk" in q or "risk" in q or "churn" in q:
        ...
        result = {
            ...
            "at_risk_rate": round(at_risk["account_id"].nunique() / total_accounts, 3) if total_accounts else 0,
        ...
        # Extract the account ID — expect format like "account ACC0042" or just "ACC0042"
        tokens = q.upper().split()
        acct_id = next((t for t in tokens if re.match(r"^ACC\d+$", t)), None)
        if acct_id is None:
            return json.dumps({"error": "Could not parse account ID. Use format: 'account ACC0042'"})
        ...
        result = {
            ...
            "avg_products_per_account": round(usage.groupby("account_id")["product_name"].nunique().mean(), 2) if total_accounts else 0,
...
def get_deals_by_stage(query: str = "all") -> str:
    """
    Retrieve CRM deals organized by pipeline stage or filtered by deal type.
    Examples:
      - 'all'                → deal counts and pipeline value by stage
      - 'closing this month' → open deals with close dates in the current month
      - 'high value'         → top 20% deals by pipeline value
      - 'stalled deals'      → open deals with no stage movement in 60+ days
    Returns: deal counts, pipeline value, and deal details.
    """
    opps = _load("opportunities.csv")
    accounts = _load("accounts.csv")
    ...
    if "closing this month" in q or "this month" in q:
        ...
        open_deals = merged[~merged["stage"].isin(["Closed Won", "Closed Lost"])]
        if len(open_deals) == 0:
            return json.dumps({"message": "No open deals found"})
        threshold = open_deals["pipeline_value"].quantile(0.8)
        high_val = open_deals[open_deals["pipeline_value"] > threshold].sort_values("pipeline_value", ascending=False)
...
def get_company_context(query: str) -> str:
    ...
    Always use this tool to ground recommendations in company-specific knowledge.
    Examples:
      - 'ideal customer profile'      → ICP details, firmographics, buyer personas
      - 'growth strategy targets'     → revenue targets, expansion goals
      - 'pricing and discounting rules'→ pricing tiers, discount thresholds
